...
interface ScriptUploadProps {
  onScriptReady?: (content: string) => void;  // NEW PROP
}

export default function ScriptUpload({ onScriptReady }: ScriptUploadProps) {
  const [scriptContent, setScriptContent] = useState<string>('');
  const [fileName, setFileName] = useState<string>('');
  const [activeTab, setActiveTab] = useState<string>('type');
  const fileInputRef = useRef<HTMLInputElement>(null);

  // Whenever scriptContent changes, notify parent
  useEffect(() => {
    if (onScriptReady) {
      onScriptReady(scriptContent);
    }
  }, [scriptContent, onScriptReady]);
          ...
          <TabsContent value="type" className="space-y-4">
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) => setScriptContent(e.target.value)}
                className="min-h-[150px]"
                placeholder="Type or paste your reference script here..."
              />
            </div>
            {scriptContent && (
              <div className="flex justify-end">
